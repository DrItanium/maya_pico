...
void ObjectMatchDelay(
    ...
    if (!UDFFirstArgument(context, ANY_TYPE_BITS, returnValue)) { return; }
    ...
    while (current_alpha_link != NULL) {
        ...
        if (slotNameIDs == NULL) {
            ...
            for (upper = alphaPtr->patternNode;
                ...
                if (upper->matchTimeTag == ObjectReteData(theEnv)->CurrentObjectMatchTimeTag) { break; }
                else { upper->matchTimeTag = ObjectReteData(theEnv)->CurrentObjectMatchTimeTag; }
            }
        }
        ...
        if (patternTop->matchTimeTag == ObjectReteData(theEnv)->CurrentObjectMatchTimeTag) {
            ...
            if ((patternTop->slotNameID == ISA_ID) ||
                ...
                ObjectReteData(theEnv)->CurrentPatternObjectSlot =
                        ObjectReteData(theEnv)->CurrentPatternObject->slotAddresses[
                                ObjectReteData(theEnv)->CurrentPatternObject->cls->slotNameMap
...
static void ProcessPatternNode(
    ...
    if (ObjectReteData(theEnv)->CurrentPatternObjectSlot == NULL) {
        if (patternNode->selector) /* TBD Necessary? */
        {
            if (EvaluateObjectPatternTest(theEnv, objectSlotField, NULL, patternNode->networkTest->nextArg, patternNode)) {
                ...
                if (tempPtr != NULL) {
                    if (tempPtr->alphaNode != NULL) { CreateObjectAlphaMatch(theEnv, tempPtr->alphaNode); }
    ...
    if (patternNode->endSlot == false) {
        ...
        if ((objectSlotLength + 2) < (newMark->startPosition + patternNode->leaveFields)) { repeatCount = 0; }
        else { repeatCount = objectSlotLength + 2 - newMark->startPosition - patternNode->leaveFields; }
