...
void SortFunction(
    ...
    if (!UDFNthArgument(context, 1, SYMBOL_BIT, &theArg)) { return; }
    ...
    for (i = 2; i <= argumentCount; i++) {
        ...
        if (theArguments[i - 2].header->type == MULTIFIELD_TYPE) { argumentSize += theArguments[i - 2].range; }
        else { argumentSize++; }
    }
    ...
    for (i = 0; i < argumentSize; i++) { RetainUDFV(theEnv, &theArguments2[i]); }
...
static void DoMergeSort(
    ...
    if (s1 == e1) { /* List doesn't need to be merged. */ }
    else if ((s1 + 1) == e1) {
    ...
    if (s2 == e2) { /* List doesn't need to be merged. */ }
    else if ((s2 + 1) == e2) {
    ...
    for (c1 = s1; c1 <= e2; c1++) { TransferDataObjectValues(&theList[c1], &tempList[c1]); }
}
