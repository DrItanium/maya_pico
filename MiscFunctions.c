...
void ExitCommand(
    ...
    if (argCnt == 0) { ExitRouter(theEnv, EXIT_SUCCESS); }
    else {
...
void SetgenFunction(
    ...
    if (!UDFNthArgument(context, 1, INTEGER_BIT, returnValue)) { return; }
...
void SeedFunction(
    ...
    if (!UDFFirstArgument(context, INTEGER_BIT, &theValue)) { return; }
...
void CallFunction(
    ...
    if (!UDFFirstArgument(context, SYMBOL_BIT | EXTERNAL_ADDRESS_BIT, &theValue)) { return; }
...
static int FindLanguageType(
    ...
    for (theType = 0; theType < EvaluationData(theEnv)->numberOfAddressTypes; theType++) {
        if (strcmp(EvaluationData(theEnv)->ExternalAddressTypes[theType]->name, languageName) == 0) { return (theType); }
    }
...
static void ConvertTime(
    ...
    if (info->tm_isdst > 0) { returnValue->multifieldValue->contents[8].lexemeValue = TrueSymbol(theEnv); }
    else if (info->tm_isdst == 0) { returnValue->multifieldValue->contents[8].lexemeValue = FalseSymbol(theEnv); }
    else { returnValue->multifieldValue->contents[8].lexemeValue = CreateSymbol(theEnv, "UNKNOWN"); }
}
...
void TimerFunction(
    ...
    while (UDFHasNextArgument(context) &&
           (!GetHaltExecution(theEnv))) { UDFNextArgument(context, ANY_TYPE_BITS, &theArg); }
