...
long long Run(
    ...
    while ((theActivation != NULL) &&
        ...
        if (GetSalienceEvaluation(theEnv) == EVERY_CYCLE) { RefreshAllAgendas(theEnv); }
    ...
    if (runLimit == rulesFired) { WriteString(theEnv, STDOUT, "rule firing limit reached\n"); }
...
void FocusCommand(
    ...
    for (i = argCount; i > 0; i--) {
        if (!UDFNthArgument(context, i, SYMBOL_BIT, &theArg)) { return; }
