...
struct lhsParseNode *RestrictionParse(
    ...
    while (theToken->tknType != RIGHT_PARENTHESIS_TOKEN) {
        ...
        if ((theToken->tknType == SF_WILDCARD_TOKEN) ||
            ...
            if (theToken->tknType == SF_WILDCARD_TOKEN) { nextNode->pnType = SF_WILDCARD_NODE; }
            else { nextNode->pnType = MF_WILDCARD_NODE; }
        ...
        if ((nextNode->pnType == SF_WILDCARD_NODE) ||
            (nextNode->pnType == SF_VARIABLE_NODE)) { numberOfSingleFields++; }
        else { numberOfMultifields++; }
    ...
    for (nextNode = topNode; nextNode != NULL; nextNode = nextNode->right) {
        ...
        if ((nextNode->pnType != MF_WILDCARD_NODE) && (nextNode->pnType != MF_VARIABLE_NODE)) { continue; }
...
static void TallyFieldTypes(
    ...
    for (tempNode1 = theRestrictions; tempNode1 != NULL; tempNode1 = tempNode1->right) {
        ...
        if ((tempNode1->pnType == SF_VARIABLE_NODE) || (tempNode1->pnType == SF_WILDCARD_NODE)) { runningSingleFields++; }
        else { runningMultiFields++; }
    }
}
