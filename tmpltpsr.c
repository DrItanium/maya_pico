...
bool ParseDeftemplate(
    ...
    if (GetConserveMemory(theEnv) == true) { newDeftemplate->header.ppForm = NULL; }
    ...
    if ((BitwiseTest(DeftemplateData(theEnv)->DeletedTemplateDebugFlags, 0)) ||
        (GetWatchItem(theEnv, "facts") == 1)) { DeftemplateSetWatch(newDeftemplate, true); }
#endif
...
static struct templateSlot *SlotDeclarations(
    ...
    while (inputToken->tknType != RIGHT_PARENTHESIS_TOKEN) {
        ...
        if (newSlot != NULL) {
            if (lastSlot == NULL) { slotList = newSlot; }
            else { lastSlot->next = newSlot; }
            lastSlot = newSlot;
        }
...
static struct templateSlot *DefinedSlots(
    ...
    if (multifieldSlot) { newSlot->constraints->multifieldsAllowed = true; }
...
static bool ParseFacetAttribute(
    ...
    } else { facetPair->argList = facetValue; }
