...
void ReadNeededSymbols(
    ...
    for (i = 0; i < SymbolData(theEnv)->NumberOfSymbols; i++) {
        if (types[i] == SYMBOL_TYPE) { SymbolData(theEnv)->SymbolArray[i] = CreateSymbol(theEnv, namePtr); }
        else if (types[i] == STRING_TYPE) { SymbolData(theEnv)->SymbolArray[i] = CreateString(theEnv, namePtr); }
        else { SymbolData(theEnv)->SymbolArray[i] = CreateInstanceName(theEnv, namePtr); }
...
void ReadNeededFloats(
    ...
    for (i = 0; i < SymbolData(theEnv)->NumberOfFloats; i++) { SymbolData(theEnv)->FloatArray[i] = CreateFloat(theEnv, floatValues[i]); }
...
#endif /* BLOAD || BLOAD_AND_BSAVE || BLOAD_INSTANCES || BSAVE_INSTANCES */
