...
void MakeInstanceCommand(
    ...
    if (temp.header->type == INSTANCE_NAME_TYPE) { iname = temp.lexemeValue; }
    else if (temp.header->type == SYMBOL_TYPE) { iname = CreateInstanceName(theEnv, temp.lexemeValue->contents); }
    else {
...
void InitSlotsCommand(
    ...
    if (!EvaluationData(theEnv)->EvaluationError) { returnValue->instanceValue = GetActiveInstance(theEnv); }
...
InstanceBuilder *CreateInstanceBuilder(
    ...
    if ((theDefclass == NULL) || (theDefclass->slotCount == 0)) { theIB->ibValueArray = NULL; }
        ...
        for (i = 0; i < theDefclass->slotCount; i++) { theIB->ibValueArray[i].voidValue = VoidConstant(theEnv); }
    }
...
PutSlotError IBPutSlot(
    ...
    if (whichSlot == -1) { return PSE_SLOT_NOT_FOUND_ERROR; }
    ...
    if (((theSlot->multiple == 0) && (slotValue->header->type == MULTIFIELD_TYPE)) ||
        ((theSlot->multiple == 1) && (slotValue->header->type != MULTIFIELD_TYPE))) { return PSE_CARDINALITY_ERROR; }
...
void IBAbort(
    ...
    for (i = 0; i < theIB->ibDefclass->slotCount; i++) {
        ...
        if (theIB->ibValueArray[i].header->type == MULTIFIELD_TYPE) { ReturnMultifield(theEnv, theIB->ibValueArray[i].multifieldValue); }
...
InstanceModifier *CreateInstanceModifier(
    ...
    if ((oldInstance == NULL) || (oldInstance->cls->slotCount == 0)) {
        ...
        for (i = 0; i < oldInstance->cls->slotCount; i++) { theIM->imValueArray[i].voidValue = VoidConstant(theEnv); }
...
PutSlotError IMPutSlot(
    ...
    if (theIM->imOldInstance->garbage) { return PSE_INVALID_TARGET_ERROR; }
    ...
    if (oldInstanceValue.header->type == MULTIFIELD_TYPE) {
        if (MultifieldsEqual(oldInstanceValue.multifieldValue, slotValue->multifieldValue)) {
            ...
            if (oldValue.header->type == MULTIFIELD_TYPE) { ReturnMultifield(theIM->imEnv, oldValue.multifieldValue); }
...
static bool IMModifySlots(
    ...
    for (i = 0; i < theInstance->cls->slotCount; i++) {
        ...
        } else { CLIPSToUDFValue(&overrides[i], &temp); }

        if (PutSlotValue(theEnv, theInstance, insSlot, &temp, &junk, "InstanceModifier call") != PSE_NO_ERROR) { return false; }
    }
...
void IMAbort(
    ...
    for (i = 0; i < theIM->imOldInstance->cls->slotCount; i++) {
        ...
        if (theIM->imValueArray[i].header->type == MULTIFIELD_TYPE) { ReturnMultifield(theEnv, theIM->imValueArray[i].multifieldValue); }
...
InstanceModifierError IMSetInstance(
    ...
    if (newSlotCount != 0) { ClearBitString((void *) theIM->changeMap, CountToBitMapSize(theIM->imOldInstance->cls->slotCount)); }
