...
static bool PropagateVariableDriver(
    ...
    if (PropagateVariableToNodes(theEnv, patternHead->expression, theType, variableName, theReference,
                                 patternHead->beginNandDepth, assignReference, true)) { return true; }
...
static bool ProcessField(
    ...
    if (UnboundVariablesInPattern(theEnv, thePattern, patternHead->whichCE)) { return true; }
    ...
    if (ProcessConnectedConstraints(theEnv, thePattern, multifieldHeader, patternHead)) { return true; }
        ...
        if (theNode->secondaryExpression != NULL) {
            ...
            if ((theReference->constraints != NULL) && (!theNode->negated)) {
                ...
                theNode->constraints = IntersectConstraints(theEnv, theReference->constraints,
                                                            tempConstraints);
                if (theNode->derivedConstraints) { RemoveConstraint(theEnv, tempConstraints); }
