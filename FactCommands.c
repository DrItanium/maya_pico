...
void RetractCommand(
    ...
    while (UDFHasNextArgument(context)) {
        ...
        if (!UDFNextArgument(context, INTEGER_BIT | FACT_ADDRESS_BIT | SYMBOL_BIT, &theArg)) { return; }
...
void FactIndexFunction(
    ...
    if (!UDFFirstArgument(context, FACT_ADDRESS_BIT, &theArg)) { return; }
...
static long long GetFactsArgument(
    ...
    if (!UDFNextArgument(context, INTEGER_BIT, &theArg)) { return (INVALID); }
