...
static bool GetVariableDefinition(
    ...
    } else { ReturnExpression(theEnv, assignPtr); }
    ...
    if (!ConstructData(theEnv)->CheckSyntaxMode) { AddDefglobal(theEnv, variableName, &assignValue, assignPtr); }
...
static void AddDefglobal(
    ...
    else { defglobalPtr->current.value = CopyMultifield(theEnv, vPtr->multifieldValue); }
