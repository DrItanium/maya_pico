...
void PrintFactIdentifierInLongForm(
    ...
    } else { WriteString(theEnv, logicalName, "<Dummy Fact>"); }
...
Fact *AssertDriver(
    ...
    for (i = 0; i < length; i++) {
        if (theField[i].value == VoidConstant(theEnv)) { theField[i].value = CreateSymbol(theEnv, "nil"); }
    }
    ...
    if (reuseIndex > 0) { theFact->factIndex = reuseIndex; }
    else { theFact->factIndex = FactData(theEnv)->NextFactIndex++; }
    ...
    if (EvaluationData(theEnv)->EvaluationError) { FactData(theEnv)->assertError = AE_RULE_NETWORK_ERROR; }
...
Fact *CreateFact(
    ...
    if (theDeftemplate->implied == false) {
        ...
        for (i = 0;
             ...
             i++) { newFact->theProposition.contents[i].voidValue = VoidConstant(theEnv); }
    }
...
bool PutFactSlot(
    ...
    if (((theSlot->multislot == 0) && (theValue->header->type == MULTIFIELD_TYPE)) ||
        ((theSlot->multislot == 1) && (theValue->header->type != MULTIFIELD_TYPE))) { return false; }
...
bool DeftemplateSlotDefault(
    ...
    if (slotPtr->noDefault) return false;
        ...
        if (!EvaluateAndStoreInDataObject(theEnv, slotPtr->multislot,
                                          ...
                                          theResult, garbageMultifield)) { return false; }
    }
...
bool CopyFactSlotValues(
    ...
    if (theDestFact->whichDeftemplate != theDeftemplate) { return false; }
...
PutSlotError FBPutSlot(
    ...
    if (((theSlot->multislot == 0) && (slotValue->header->type == MULTIFIELD_TYPE)) ||
        ((theSlot->multislot == 1) && (slotValue->header->type != MULTIFIELD_TYPE))) { return PSE_CARDINALITY_ERROR; }
    ...
    if (oldValue.header->type == MULTIFIELD_TYPE) {
        if (MultifieldsEqual(oldValue.multifieldValue, slotValue->multifieldValue)) { return PSE_NO_ERROR; }
    } else {
        if (oldValue.value == slotValue->value) { return PSE_NO_ERROR; }
    }

    Release(theEnv, oldValue.header);

    if (oldValue.header->type == MULTIFIELD_TYPE) { ReturnMultifield(theEnv, oldValue.multifieldValue); }
...
void FBAbort(
    ...
    for (i = 0; i < theFB->fbDeftemplate->numberOfSlots; i++) {
        ...
        if (theFB->fbValueArray[i].header->type == MULTIFIELD_TYPE) { ReturnMultifield(theEnv, theFB->fbValueArray[i].multifieldValue); }
...
FactModifier *CreateFactModifier(
    ...
    if ((oldFact == NULL) || (oldFact->whichDeftemplate->numberOfSlots == 0)) {
        ...
        for (i = 0; i < oldFact->whichDeftemplate->numberOfSlots; i++) { theFM->fmValueArray[i].voidValue = VoidConstant(theEnv); }
...
PutSlotError FMPutSlot(
    ...
    if (theFM->fmOldFact->garbage) { return PSE_INVALID_TARGET_ERROR; }
    ...
    if (oldFactValue.header->type == MULTIFIELD_TYPE) {
        if (MultifieldsEqual(oldFactValue.multifieldValue, slotValue->multifieldValue)) {
            ...
            if (oldValue.header->type == MULTIFIELD_TYPE) { ReturnMultifield(theFM->fmEnv, oldValue.multifieldValue); }
...
Fact *FMModify(
    ...
    if ((FactData(theEnv)->assertError == AE_RULE_NETWORK_ERROR) ||
        (FactData(theEnv)->retractError == RE_RULE_NETWORK_ERROR)) { FactData(theEnv)->factModifierError = FME_RULE_NETWORK_ERROR; }
...
void FMAbort(
    ...
    for (i = 0; i < theFM->fmOldFact->whichDeftemplate->numberOfSlots; i++) {
        ...
        if (theFM->fmValueArray[i].header->type == MULTIFIELD_TYPE) { ReturnMultifield(theEnv, theFM->fmValueArray[i].multifieldValue); }
...
FactModifierError FMSetFact(
    ...
    for (i = 0; i < newSlotCount; i++) { theFM->fmValueArray[i].voidValue = theFM->fmEnv->VoidConstant; }

    if (newSlotCount != 0) { ClearBitString((void *) theFM->changeMap, CountToBitMapSize(newSlotCount)); }
