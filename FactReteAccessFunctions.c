...
bool FactPNGetVar3(
    ...
    if (hack->fromBeginning) { fieldPtr = &segmentPtr->contents[hack->beginOffset]; }
    else { fieldPtr = &segmentPtr->contents[segmentPtr->length - (hack->endOffset + 1)]; }
...
#pragma unused(returnValue)
    ...
    if (fieldPtr->header->type == MULTIFIELD_TYPE) {
        ...
        if (hack->fromBeginning) { fieldPtr = &segmentPtr->contents[hack->offset]; }
        else {
...
bool FactSlotLength(
    ...
    if (segmentPtr->length < (hack->minLength + extraOffset)) { return false; }

    if (hack->exactly && (segmentPtr->length > (hack->minLength + extraOffset))) { return false; }
...
#pragma unused(theResult)
    ...
    if (hack->p2rhs) { fact2 = (Fact *) EngineData(theEnv)->GlobalRHSBinds->binds[p2].gm.theMatch->matchingItem; }
    else { fact2 = (Fact *) EngineData(theEnv)->GlobalLHSBinds->binds[p2].gm.theMatch->matchingItem; }
    ...
    if (fact1->theProposition.contents[e1].value !=
        fact2->theProposition.contents[e2].value) { return hack->fail; }
...
#pragma unused(theResult)
    ...
    if (fact1->theProposition.contents[s1].header->type != MULTIFIELD_TYPE) { fieldPtr1 = &fact1->theProposition.contents[s1]; }
    else {
        segment = fact1->theProposition.contents[s1].multifieldValue;

        if (hack->fromBeginning1) { fieldPtr1 = &segment->contents[hack->offset1]; }
        else { fieldPtr1 = &segment->contents[segment->length - (hack->offset1 + 1)]; }
    }

    if (fact2->theProposition.contents[s2].header->type != MULTIFIELD_TYPE) { fieldPtr2 = &fact2->theProposition.contents[s2]; }
    else {
        segment = fact2->theProposition.contents[s2].multifieldValue;

        if (hack->fromBeginning2) { fieldPtr2 = &segment->contents[hack->offset2]; }
        else { fieldPtr2 = &segment->contents[segment->length - (hack->offset2 + 1)]; }
    }
    ...
    if (fieldPtr1->value != fieldPtr2->value) { return hack->fail; }
...
#pragma unused(theEnv)
    ...
    for (;
        ...
        else if (markList->whichField > whichField) { return actualIndex; }
