...
bool LoadConstructsFromLogicalName(
    ...
    while ((foundConstruct == true) && (GetHaltExecution(theEnv) == false)) {
        ...
        if (foundConstruct) { ReleaseLexeme(theEnv, theToken.lexemeValue); }
    }
...
static bool FindConstructBeginning(
    ...
    while (theToken->tknType != STOP_TOKEN) {
        ...
        if (theToken->tknType == LEFT_PARENTHESIS_TOKEN) { leftParenthesisFound = true; }
...
void DeleteErrorCaptureRouter(
    ...
    if (ConstructData(theEnv)->errorCaptureRouterCount == 0) { DeleteRouter(theEnv, "error-capture"); }
}
...
BuildError ParseConstruct(
    ...
    if ((*currentPtr->parseFunction)(theEnv, logicalName)) { rv = BE_PARSING_ERROR; }
    else { rv = BE_NO_ERROR; }
