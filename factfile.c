...
void SaveFactsCommand(
    ...
    if (numArgs > 1) {
        ...
        if (strcmp(argument, "local") == 0) { saveCode = LOCAL_SAVE; }
        else if (strcmp(argument, "visible") == 0) { saveCode = VISIBLE_SAVE; }
        else {
...
long SaveFactsDriver(
    ...
    for (theFact = GetNextFactInScope(theEnv, NULL);
        ...
        if ((saveCode == LOCAL_SAVE) &&
            (theFact->whichDeftemplate->header.whichModule->theModule != theModule)) { printFact = false; }
...
static void CreateSlotValue(
    ...
    if (isMultislot) {
        ...
        for (i = 0; i < valueCount; i++) { returnValue->multifieldValue->contents[i].value = GetBinaryAtomValue(theEnv, &bsaValues[i]); }
    } else { returnValue->value = GetBinaryAtomValue(theEnv, &bsaValues[0]); }
}
...
static long MarkFacts(
    ...
    for (theFact = GetNextFactInScope(theEnv, NULL);
        ...
        if ((saveCode == LOCAL_SAVE) &&
            (theFact->whichDeftemplate->header.whichModule->theModule != theModule)) { markFact = false; }
    ...
    if (theFact->whichDeftemplate->implied) {
        ...
        *neededSpace += (sizeof(unsigned short) + // Number of slots
        ...
        for (i = 0; i < theFact->whichDeftemplate->numberOfSlots; i++) {
            ...
            } else { MarkNeededAtom(theEnv, theValue, neededSpace); }
            sp = sp->next;
        }
    }
}
...
static long SaveBinaryFacts(
    ...
    for (theFact = GetNextFactInScope(theEnv, NULL);
        ...
        if ((saveCode == LOCAL_SAVE) &&
            (theFact->whichDeftemplate->header.whichModule->theModule != theModule)) { saveFact = false; }
...
static void SaveSingleFactBinary(
    ...
    if (theFact->whichDeftemplate->implied) { slotCount = 1; }
    else { slotCount = theFact->whichDeftemplate->numberOfSlots; }
    ...
    if (slotCount != 0) { fwrite(&totalValueCount, sizeof(unsigned long), 1, filePtr); }
    ...
    if (theFact->whichDeftemplate->implied) {
        ...
        for (i = 0; i < slotCount; i++) {
            ...
            if (sp->multislot) {
                for (j = 0; j < slotLen; j++) { SaveAtomBinary(theEnv, &theValue->multifieldValue->contents[j], filePtr); }
            } else { SaveAtomBinary(theEnv, theValue, filePtr); }
