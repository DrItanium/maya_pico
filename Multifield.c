...
void IncrementCLIPSValueMultifieldReferenceCount(
    ...
    for (i = 0; i < length; i++) { Retain(theEnv, contents[i].header); }
}
...
void DecrementCLIPSValueMultifieldReferenceCount(
    ...
    for (i = 0; i < length; i++) { Release(theEnv, contents[i].header); }
}
...
Multifield *StringToMultifield(
    ...
    while (theToken.tknType != STOP_TOKEN) {
        if ((theToken.tknType == SYMBOL_TOKEN) || (theToken.tknType == STRING_TOKEN) ||
            ...
            (theToken.tknType == INSTANCE_NAME_TOKEN)) { theAtom = GenConstant(theEnv, TokenTypeToType(theToken.tknType), theToken.value); }
        ...
        unsigned long size) // TBD size_t
{
    ...
    for (i = 0; i < size; i++) { rv->contents[i].value = theArray[i].value; }
...
void EphemerateMultifield(
    ...
    for (i = 0; i < length; i++) { EphemerateValue(theEnv, contents[i].value); }
}
...
void StoreInMultifield(
    ...
    if (argCount == 0) {
        ...
        if (garbageSegment) { theMultifield = CreateMultifield(theEnv, seg_size); }
...
bool MultifieldsEqual(
    ...
    if (length != segment2->length) { return false; }
...
void MBAppendUDFValue(
    ...
    if (theValue->header->type == MULTIFIELD_TYPE) {
        if (theValue->range == 0) { return; }

        neededSize = theMB->length + theValue->range;
    } else { neededSize = theMB->length + 1; }
    ...
    if (neededSize > theMB->bufferMaximum) {
        ...
        for (i = 0; i < theMB->length; i++) { newArray[i] = theMB->contents[i]; }
...
void MBAppend(
    ...
    if (theValue->header->type == MULTIFIELD_TYPE) {
        if (theValue->multifieldValue->length == 0) { return; }
