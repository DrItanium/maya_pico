...
void FactPatternMatch(
    ...
    while (patternPtr != NULL) {
        ...
        if (offsetSlot == patternPtr->whichSlot) { theSlotField = theSlotField + offset - multifieldsProcessed; }
        ...
        if (SkipFactPatternNode(theEnv, patternPtr)) { patternPtr = GetNextFactPatternNode(theEnv, true, patternPtr); }
            ...
            if (patternPtr->header.endSlot &&
                ...
                if ((patternPtr->leaveFields + theSlotField) !=
                    FactData(theEnv)->CurrentPatternFact->theProposition.contents
                    [patternPtr->whichSlot].multifieldValue->length) { skipit = true; }
            }

            if (skipit) { patternPtr = GetNextFactPatternNode(theEnv, true, patternPtr); }
                ...
                } else { patternPtr = GetNextFactPatternNode(theEnv, true, patternPtr); }
            }
                ...
                if (patternPtr->header.stopNode) { ProcessFactAlphaMatch(theEnv, theFact, markers, patternPtr); }
...
static void ProcessMultifieldNode(
    ...
    if (thePattern->header.endSlot) {
        if (theSlotValue->length < (newMark->startPosition + thePattern->leaveFields)) { newMark->range = 0; }
        else { newMark->range = theSlotValue->length - (newMark->startPosition + thePattern->leaveFields); }
