...
static void IntersectNumericExpressions(
    ...
    for (;
        ...
        for (;
            ...
            if ((cmaxmin == LESS_THAN) || (cminmax == GREATER_THAN)) { continue; }
            ...
            if (cminmin == GREATER_THAN) { theMin = GenConstant(theEnv, tmpmin1->type, tmpmin1->value); }
            ...
            if (cmaxmax == LESS_THAN) { theMax = GenConstant(theEnv, tmpmax1->type, tmpmax1->value); }
...
static void UnionRangeMinMaxValueWithList(
    ...
    while (tmpmin != NULL) {
        ...
        if (((cmaxmax == LESS_THAN) || (cmaxmax == EQUAL)) &&
            ((cminmin == GREATER_THAN) || (cminmin == EQUAL))) { return; }
