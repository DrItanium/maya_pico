...
static void GenObjectGetVar(
    ...
    if (theNode->slotNumber == UNSPECIFIED_SLOT) {
        ...
        if (joinReference) { theItem->type = OBJ_GET_SLOT_JNVAR1; }
        else { theItem->type = OBJ_GET_SLOT_PNVAR1; }
...
static Expression *GenerateSlotComparisonTest(
    ...
    if (isNand) { firstNode = referringNode; }
    ...
    if (IsSimpleSlotVariable(firstNode) && IsSimpleSlotVariable(referringNode)) {
        ...
        if ((firstNode->withinMultifieldSlot == false) &&
            ...
            if (joinTest) {
                if (isNand) { jhack1.firstPattern = referringNode->joinDepth; }
                else { jhack1.firstPattern = 0; }
            ...
            if (firstNode->withinMultifieldSlot == true) {
                ...
                if (joinTest) {
                    if (isNand) { jhack2.firstPattern = referringNode->joinDepth; }
                    else { jhack2.firstPattern = 0; }
                ...
                if (joinTest) {
                    if (isNand) { jhack2.secondPattern = firstNode->joinDepth; }
                    else { jhack2.secondPattern = 0; }
            ...
            if (joinTest) {
                if (isNand) { jhack3.firstPattern = referringNode->joinDepth; }
                else { jhack3.firstPattern = 0; }
