...
struct expr *FactGenGetfield(
    ...
    if ((theNode->slotNumber > 0) && (theNode->slotNumber != UNSPECIFIED_SLOT) &&
        (theNode->withinMultifieldSlot == false)) { return (GenConstant(theEnv, FACT_PN_VAR2, FactGetVarPN2(theEnv, theNode))); }
...
struct expr *FactGenGetvar(
    ...
    if ((theNode->slotNumber > 0) && (theNode->slotNumber != UNSPECIFIED_SLOT) &&
        (theNode->withinMultifieldSlot == false)) { return (GenConstant(theEnv, FACT_JN_VAR2, FactGetVarJN2(theEnv, theNode, side))); }
    ...
    if (((theNode->pnType == MF_WILDCARD_NODE) || (theNode->pnType == MF_VARIABLE_NODE)) &&
        (theNode->multiFieldsBefore == 0) &&
        (theNode->multiFieldsAfter == 0)) { return (GenConstant(theEnv, FACT_JN_VAR3, FactGetVarJN3(theEnv, theNode, side))); }
...
struct expr *FactGenCheckLength(
    ...
    if ((theNode->pnType != MF_VARIABLE_NODE) &&
        ...
        (theNode->multiFieldsAfter == 0)) { hack.exactly = 1; }
    else { hack.exactly = 0; }
    ...
    if ((theNode->pnType == SF_VARIABLE_NODE) || (theNode->pnType == SF_WILDCARD_NODE)) { hack.minLength = 1 + theNode->singleFieldsAfter; }
    else { hack.minLength = theNode->singleFieldsAfter; }
...
static void *FactGetVarJN1(
    ...
    if (side == CLIPS_LHS) {
        ...
        hack.rhs = 1;
        hack.whichPattern = theNode->joinDepth;
    } else { hack.whichPattern = theNode->joinDepth; }
...
struct expr *FactJNVariableComparison(
    ...
    if ((selfNode->withinMultifieldSlot == false) &&
        ...
        if (nandJoin) { firstNode = referringNode; }
        else { firstNode = selfNode; }

        hack1.slot1 = firstNode->slotNumber - 1;

        hack1.p1rhs = true;
        if (nandJoin) { hack1.pattern1 = referringNode->joinDepth; }
        else { hack1.pattern1 = 0; }
        ...
        if (nandJoin) { hack2.pattern1 = referringNode->joinDepth; }
        else { hack2.pattern1 = 0; }
        ...
        if (nandJoin) { top->argList = FactGenGetvar(theEnv, selfNode, NESTED_RHS); }
        else { top->argList = FactGenGetvar(theEnv, selfNode, CLIPS_RHS); }
