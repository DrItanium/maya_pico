...
bool ParseDefrule(
    ...
    SavePPBuffer(theEnv, "\n");
    if (GetConserveMemory(theEnv) == true) { topDisjunct->header.ppForm = NULL; }
...
#if DEBUGGING_FUNCTIONS
    if (BitwiseTest(DefruleData(theEnv)->DeletedRuleDebugFlags, 0)) { SetBreak(topDisjunct); }
    if (BitwiseTest(DefruleData(theEnv)->DeletedRuleDebugFlags, 1) ||
        (GetWatchItem(theEnv, "activations") == 1)) { DefruleSetWatchActivations(topDisjunct, true); }
    if (BitwiseTest(DefruleData(theEnv)->DeletedRuleDebugFlags, 2) ||
        (GetWatchItem(theEnv, "rules") == 1)) { DefruleSetWatchFirings(topDisjunct, true); }
#endif
...
static int ReplaceRHSVariable(
    ...
    if ((list->type != SF_VARIABLE) && (list->type != MF_VARIABLE)) { return 0; }
    ...
    if (theVariable->patternType != NULL) { (*theVariable->patternType->replaceGetJNValueFunction)(theEnv, list, theVariable, CLIPS_LHS); }
    else { return 0; }
...
static unsigned short RuleComplexity(
    ...
    while (theLHS != NULL) {
        ...
        while (thePattern != NULL) {
            ...
            } else { complexity += ExpressionComplexity(theEnv, thePattern->networkTest); }
    ...
    while (exprPtr != NULL) {
        if (exprPtr->type == FCALL) {
            ...
            if ((exprPtr->value == ExpressionData(theEnv)->PTR_AND) ||
                ...
                (exprPtr->value == ExpressionData(theEnv)->PTR_OR)) { complexity += ExpressionComplexity(theEnv, exprPtr->argList); }
                ...
                /*=========================================*/
                   ...
                   EvaluationData(theEnv)->PrimitivesArray[exprPtr->type]->addsToRuleComplexity : false) { complexity++; }
...
static int LogicalAnalysis(
    ...
    for (;
        ...
        if (((patternList->pnType != PATTERN_CE_NODE) &&
            ...
            (patternList->endNandDepth != 1)) { continue; }
        ...
        if (patternList->pnType == PATTERN_CE_NODE) { logicalJoin++; }
        else if (logicalJoin == 1) { logicalJoin++; }
...
struct lhsParseNode *FindVariable(
    ...
    for (;
        ...
        if ((theLHS->pnType != PATTERN_CE_NODE) ||
            (theLHS->negated == true) ||
            (theLHS->exists == true) ||
            (theLHS->beginNandDepth > 1)) { continue; }

        /*=====================================*/
        /* Check the pattern address variable. */
        /*=====================================*/

        if (theLHS->value == (void *) name) { theReturnValue = theLHS; }
        ...
        while (theFields != NULL) {
            ...
            if (theFields == NULL) { /* Do Nothing */ }
            else if (((theFields->pnType == SF_VARIABLE_NODE) ||
                      (theFields->pnType == MF_VARIABLE_NODE)) &&
                     (theFields->value == (void *) name)) { theReturnValue = theFields; }
...
static void AddToDefruleList(
    ...
    if (theModuleItem->header.lastItem == NULL) { theModuleItem->header.firstItem = &rulePtr->header; }
    else {
