...
static void StrOrSymCatFunction(
    ...
    for (i = 1; i <= numArgs; i++) {
        ...
        if (EvaluationData(theEnv)->EvaluationError) {
            ...
            if (returnType == STRING_TYPE) { returnValue->value = CreateString(theEnv, ""); }
            else { returnValue->value = CreateSymbol(theEnv, "nil"); }
            return;
        }
    ...
    if (returnType == STRING_TYPE) { returnValue->value = CreateString(theEnv, theString); }
    else { returnValue->value = CreateSymbol(theEnv, theString); }
...
void StrLengthFunction(
    ...
    if (!UDFFirstArgument(context, LEXEME_BITS | INSTANCE_NAME_BIT, &theArg)) { return; }
...
void UpcaseFunction(
    ...
    for (i = 0; i < slen; i++) {
        if (islower(osptr[i])) { nsptr[i] = (char) toupper(osptr[i]); }
        else { nsptr[i] = osptr[i]; }
    }
...
void LowcaseFunction(
    ...
    for (i = 0; i < slen; i++) {
        if (isupper(osptr[i])) { nsptr[i] = (char) tolower(osptr[i]); }
        else { nsptr[i] = osptr[i]; }
    }
...
void StrCompareFunction(
    ...
    if (!UDFFirstArgument(context, LEXEME_BITS | INSTANCE_NAME_BIT, &arg1)) { return; }

    if (!UDFNextArgument(context, LEXEME_BITS | INSTANCE_NAME_BIT, &arg2)) { return; }
    ...
    if (UDFHasNextArgument(context)) {
        if (!UDFNextArgument(context, INTEGER_BIT, &arg3)) { return; }

        compareResult = strncmp(arg1.lexemeValue->contents, arg2.lexemeValue->contents,
                                (STD_SIZE) arg3.integerValue->contents);
    } else { compareResult = strcmp(arg1.lexemeValue->contents, arg2.lexemeValue->contents); }
    ...
    if (compareResult < 0) { returnValue->integerValue = CreateInteger(theEnv, -1L); }
    else if (compareResult > 0) { returnValue->integerValue = CreateInteger(theEnv, 1L); }
    else { returnValue->integerValue = CreateInteger(theEnv, 0L); }
}
...
void SubStringFunction(
    ...
    if (theArg.integerValue->contents < 1) { start = 0; }
    else { start = (size_t) theArg.integerValue->contents - 1; }
    ...
    if (theArg.integerValue->contents < 1) {
        ...
        return;
    } else { end = (size_t) theArg.integerValue->contents - 1; }

    if (!UDFNextArgument(context, LEXEME_BITS | INSTANCE_NAME_BIT, &theArg)) { return; }
    ...
    if ((start > end) || (length == 0)) {
        ...
        for (j = 0, i = start; i <= end; i++, j++) { *(returnString + j) = *(tempString + i); }
        *(returnString + j) = '\0';
    }
...
void StrIndexFunction(
    ...
    if (!UDFFirstArgument(context, LEXEME_BITS | INSTANCE_NAME_BIT, &theArg1)) { return; }

    if (!UDFNextArgument(context, LEXEME_BITS | INSTANCE_NAME_BIT, &theArg2)) { return; }
...
void StringToField(
    ...
    if ((theToken.tknType == FLOAT_TOKEN) || (theToken.tknType == STRING_TOKEN) ||
        ...
        SetErrorValue(theEnv, &CreateSymbol(theEnv, "INVALID_ARGUMENT")->header);
        returnValue->lexemeValue = FalseSymbol(theEnv);
    } else { returnValue->value = CreateSymbol(theEnv, theToken.printForm); }
}
...
void StrReplaceFunction(
    ...
    if (!UDFFirstArgument(context, LEXEME_BITS | INSTANCE_NAME_BIT, &initial)) { return; }

    if (!UDFNextArgument(context, LEXEME_BITS | INSTANCE_NAME_BIT, &find)) { return; }

    if (!UDFNextArgument(context, LEXEME_BITS | INSTANCE_NAME_BIT, &replace)) { return; }
    ...
    if (initial.header->type == STRING_TYPE) { returnValue->value = CreateString(theEnv, returnString); }
    else if (initial.header->type == SYMBOL_TYPE) { returnValue->value = CreateSymbol(theEnv, returnString); }
    else { returnValue->value = CreateInstanceName(theEnv, returnString); }
...
void EvalFunction(
    ...
    if (!UDFFirstArgument(context, LEXEME_BITS, &theArg)) { return; }
...
BuildError Build(
    ...
    if (OpenStringSource(theEnv, "build", theString, 0) == 0) { return BE_COULD_NOT_BUILD_ERROR; }
