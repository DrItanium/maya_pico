...
static void PrintDriver(
    ...
    while (UDFHasNextArgument(context)) {
        if (!UDFNextArgument(context, ANY_TYPE_BITS, &theArg)) { break; }
        ...
        switch (theArg.header->type) {
            case SYMBOL_TYPE:
                if (strcmp(theArg.lexemeValue->contents, "crlf") == 0) {
                    if (IOFunctionData(theEnv)->useFullCRLF) { WriteString(theEnv, logicalName, "\r\n"); }
                    else { WriteString(theEnv, logicalName, "\n"); }
                } else if (strcmp(theArg.lexemeValue->contents, "tab") == 0) { WriteString(theEnv, logicalName, "\t"); }
                else if (strcmp(theArg.lexemeValue->contents, "vtab") == 0) { WriteString(theEnv, logicalName, "\v"); }
                else if (strcmp(theArg.lexemeValue->contents, "ff") == 0) { WriteString(theEnv, logicalName, "\f"); }
                else { WriteString(theEnv, logicalName, theArg.lexemeValue->contents); }
                break;
...
void ReadFunction(
    ...
    if (strcmp(logicalName, STDIN) == 0) { ReadTokenFromStdin(theEnv, &theToken); }
    else { GetToken(theEnv, logicalName, &theToken); }
...
void OpenFunction(
    ...
    if (!UDFHasNextArgument(context)) { accessMode = "r"; }
    else {
        if (!UDFNextArgument(context, STRING_BIT, &theArg)) { return; }
...
void TellFunction(
    ...
    if (rv == LLONG_MIN) { returnValue->lexemeValue = FalseSymbol(theEnv); }
    else { returnValue->integerValue = CreateInteger(theEnv, rv); }
}
...
void GetCharFunction(
    ...
    if (strcmp(logicalName, STDIN) == 0) {
        if (RouterData(theEnv)->InputUngets > 0) {
            ...
            if (theChar == '\b') {
                if (RouterData(theEnv)->CommandBufferInputCount > 0) { RouterData(theEnv)->CommandBufferInputCount--; }
            } else { RouterData(theEnv)->CommandBufferInputCount++; }
...
void UngetCharFunction(
    ...
    if (strcmp(logicalName, STDIN) == 0) { RouterData(theEnv)->InputUngets++; }
    ...
    while (formatString[form_pos] != '\0') {
        if (formatString[form_pos] != '%') {
            ...
            while ((formatString[form_pos] != '%') &&
                   (formatString[form_pos] != '\0')) { form_pos++; }
...
static const char *PrintFormatFlag(
    ...
    switch (formatType) {
        ...
        case 'u':
            ...
            else { gensprintf(printBuffer, formatString, theResult.integerValue->contents); }
        ...
        case 'e':
            ...
            if (theResult.header->type == FLOAT_TYPE) { gensprintf(printBuffer, formatString, theResult.floatValue->contents); }
            else { gensprintf(printBuffer, formatString, (double) theResult.integerValue->contents); }
...
void ReadNumberFunction(
    ...
    if (strcmp(logicalName, STDIN) == 0) {
        ...
        RouterData(theEnv)->InputUngets = 0;
        RouterData(theEnv)->AwaitingInput = false;
    } else { ReadNumber(theEnv, logicalName, &theToken, false); }
    ...
    else { returnValue->value = CreateString(theEnv, theToken.printForm); }
...
static void ReadNumber(
    ...
    while (isspace(inchar) && (inchar != EOF) &&
           (!GetHaltExecution(theEnv))) { inchar = ReadRouter(theEnv, logicalName); }
