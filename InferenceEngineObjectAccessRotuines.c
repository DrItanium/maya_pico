...
static bool ObjectGetVarPNFunction1(
    ...
    hack = (struct ObjectMatchVar1 *) ((CLIPSBitMap *) theValue)->contents;
    GetObjectValueGeneral(theEnv, theResult, ObjectReteData(theEnv)->CurrentPatternObject,
                          ObjectReteData(theEnv)->CurrentPatternObjectMarks, hack);
    return true;
}
...
static void GetObjectValueGeneral(
    ...
    if (extent == SIZE_MAX) {
        if ((*insSlot)->desc->multiple) { returnValue->value = (*insSlot)->multifieldValue->contents[field - 1].value; }
        else { returnValue->value = (*insSlot)->value; }
    } else {
        returnValue->value = (*insSlot)->value;
...
static size_t CalculateSlotField(
    ...
    *extent = SIZE_MAX;
    ...
    while ((theMarkers != NULL) ? (theMarkers->where.whichSlot == theSlotName) : false) {
        ...
        if (theMarkers->whichField > theIndex) { return actualIndex; }
