...
static bool CheckFunctionReturnType(
    ...
    if (constraints->integersAllowed) { if (functionReturnType & INTEGER_BIT) return true; }

    if (constraints->multifieldsAllowed) { if (functionReturnType & MULTIFIELD_BIT) return true; }

    if (constraints->externalAddressesAllowed) { if (functionReturnType & EXTERNAL_ADDRESS_BIT) return true; }

    if (constraints->factAddressesAllowed) { if (functionReturnType & FACT_ADDRESS_BIT) return true; }

    if (constraints->instanceAddressesAllowed) { if (functionReturnType & INSTANCE_ADDRESS_BIT) return true; }
...
static bool CheckTypeConstraint(
    ...
    if ((type == VOID_TYPE) && (constraints->voidAllowed != true)) { return false; }

    if ((type == FACT_ADDRESS_TYPE) && (constraints->factAddressesAllowed != true)) { return false; }
...
bool CheckAllowedClassesConstraint(
    ...
    if ((type != INSTANCE_ADDRESS_TYPE) && (type != INSTANCE_NAME_TYPE)) { return true; }
...
void ConstraintViolationErrorMessage(
    ...
    if ((violationType == TYPE_VIOLATION) ||
        (violationType == FUNCTION_RETURN_TYPE_VIOLATION)) { WriteString(theEnv, STDERR, " does not match the allowed types"); }
    ...
    } else if (violationType == ALLOWED_VALUES_VIOLATION) { WriteString(theEnv, STDERR, " does not match the allowed values"); }
    else if (violationType == CARDINALITY_VIOLATION) { WriteString(theEnv, STDERR, " does not satisfy the cardinality restrictions"); }
    else if (violationType == ALLOWED_CLASSES_VIOLATION) { WriteString(theEnv, STDERR, " does not match the allowed classes"); }
...
static void PrintRange(
        ...
        CONSTRAINT_RECORD *theConstraint) {
    if (theConstraint->minValue->value == SymbolData(theEnv)->NegativeInfinity) { WriteString(theEnv, logicalName, SymbolData(
    ...
    if (theConstraint->maxValue->value == SymbolData(theEnv)->PositiveInfinity) { WriteString(theEnv, logicalName, SymbolData(
    ...
    if (theData->header->type == MULTIFIELD_TYPE) {
        ...
        for (i = theData->begin; i < theData->begin + theData->range; i++) {
            if ((rv = ConstraintCheckValue(theEnv, theMultifield[i].header->type,
                                           ...
                                           theConstraints)) != NO_VIOLATION) { return rv; }
        }
...
ConstraintViolationType ConstraintCheckExpressionChain(
    ...
    for (theExp = theExpression; theExp != NULL; theExp = theExp->nextArg) {
        if (ConstantType(theExp->type)) min++;
            ...
            if (restriction & MULTIFIELD_BIT) { max = -1; }
            else { min++; }
        } else max = -1;
    }
