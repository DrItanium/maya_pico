...
static void FreeTemplateValueArray(
    ...
    for (i = 0; i < templatePtr->numberOfSlots; i++) {
        if (theValueArray[i].header->type == MULTIFIELD_TYPE) { ReturnMultifield(theEnv, theValueArray[i].multifieldValue); }
    }
...
void ModifyCommand(
    ...
    if (computeResult.header->type == INTEGER_TYPE) {
        ...
        while (oldFact != NULL) {
            if (oldFact->factIndex == factNum) { break; }
            else { oldFact = oldFact->nextFact; }
        }
    ...
    else if (computeResult.header->type == FACT_ADDRESS_TYPE) { oldFact = computeResult.factValue; }
    ...
    for (i = 0; i < oldFact->theProposition.length; i++) { theValueArray[i].voidValue = VoidConstant(theEnv); }
    ...
    while (testPtr != NULL) {
        ...
        if (testPtr->type == INTEGER_TYPE) { position = testPtr->integerValue->contents; }
        else {
            found = false;
            position = 0;
            ...
            if (!found) {
                ...
                if (changeMap != NULL) { rm(theEnv, (void *) changeMap, CountToBitMapSize(templatePtr->numberOfSlots)); }
                return;
            }
        }
        ...
        if (oldFact->theProposition.contents[position].header->type != MULTIFIELD_TYPE) {
            ...
            if (oldFact->theProposition.contents[position].value != computeResult.value) {
                ...
                if (changeMap != NULL) { SetBitMap(changeMap, position); }
            }
        }
            ...
            if ((oldFact->theProposition.contents[position].header->type != computeResult.header->type) ||
                ...
                replacementCount++;
                if (changeMap != NULL) { SetBitMap(changeMap, position); }
            } else { ReturnMultifield(theEnv, computeResult.multifieldValue); }
        }
    ...
    if (replacementCount == 0) {
        if (theValueArray != NULL) { rm(theEnv, theValueArray, sizeof(void *) * templatePtr->numberOfSlots); }
        if (changeMap != NULL) { rm(theEnv, (void *) changeMap, CountToBitMapSize(templatePtr->numberOfSlots)); }
    ...
    if ((oldFact = ReplaceFact(theEnv, oldFact, theValueArray, changeMap)) != NULL) { returnValue->factValue = oldFact; }
    ...
    for (i = 0; i < oldFact->theProposition.length; i++) {
        if (theValueArray[i].voidValue != VoidConstant(theEnv)) {
            ...
            if (oldFact->theProposition.contents[i].header->type == MULTIFIELD_TYPE) {
                ...
                if (theSegment->busyCount == 0) { ReturnMultifield(theEnv, theSegment); }
                else { AddToMultifieldList(theEnv, theSegment); }
            }
...
void DuplicateCommand(
    ...
    for (i = 0; i < oldFact->theProposition.length; i++) {
        if (oldFact->theProposition.contents[i].header->type !=
            MULTIFIELD_TYPE) { newFact->theProposition.contents[i].value = oldFact->theProposition.contents[i].value; }
        else { newFact->theProposition.contents[i].value = NULL; }
    }
...
void DeftemplateSlotDefaultPFunction(
    ...
    if (defaultType == STATIC_DEFAULT) { returnValue->lexemeValue = CreateSymbol(theEnv, "static"); }
    else if (defaultType == DYNAMIC_DEFAULT) { returnValue->lexemeValue = CreateSymbol(theEnv, "dynamic"); }
...
DefaultType DeftemplateSlotDefaultP(
    ...
    if (theSlot->noDefault) { return NO_DEFAULT; }
    else if (theSlot->defaultDynamic) { return DYNAMIC_DEFAULT; }
...
bool DeftemplateSlotDefaultValue(
    ...
    if (theSlot->noDefault) { theValue->value = CreateSymbol(theEnv, "?NONE"); }
...
bool DeftemplateSlotMultiP(
    ...
    if (theDeftemplate->implied) {
        if (strcmp(slotName, "implied") == 0) { return true; }
        else {
            SetEvaluationError(theEnv, true);
            InvalidDeftemplateSlotMessage(theEnv, slotName,
                                          theDeftemplate->header.name->contents, false);
...
bool DeftemplateSlotSingleP(
    ...
    if (theDeftemplate->implied) {
        if (strcmp(slotName, "implied") == 0) { return false; }
        else {
            SetEvaluationError(theEnv, true);
            InvalidDeftemplateSlotMessage(theEnv, slotName,
                                          theDeftemplate->header.name->contents, false);
...
bool DeftemplateSlotExistP(
    ...
    else if (FindSlot(theDeftemplate, CreateSymbol(theEnv, slotName), NULL) == NULL) { return false; }
...
void DeftemplateSlotFacetExistPFunction(
    ...
    returnValue->lexemeValue = CreateBoolean(theEnv, DeftemplateSlotFacetExistP(theEnv, theDeftemplate, slotName->contents,
                                                                                facetName.lexemeValue->contents));
}
...
bool DeftemplateSlotFacetExistP(
    ...
    else if ((theSlot = FindSlot(theDeftemplate, CreateSymbol(theEnv, slotName), NULL)) == NULL) { return false; }
    ...
    for (tempFacet = theSlot->facetList;
        ...
        if (tempFacet->value == facetHN) { return true; }
    }
...
void DeftemplateSlotFacetValueFunction(
    ...
    if (!UDFNthArgument(context, 3, SYMBOL_BIT, &facetName)) { return; }
...
static CLIPSLexeme *FindTemplateForFactAddress(
    ...
    if ((thePattern->pnType != SF_WILDCARD_NODE) || (thePattern->bottom == NULL)) { return NULL; }

    thePattern = thePattern->bottom;
    if ((thePattern->pnType != SYMBOL_NODE) ||
        (thePattern->right != NULL) ||
        (thePattern->bottom != NULL)) { return NULL; }
...
static struct expr *ModAndDupParse(
    ...
    while (theToken.tknType != RIGHT_PARENTHESIS_TOKEN) {
        ...
        while (!done) {
            ...
            if (newField == NULL) { done = true; }

            if (lastField == NULL) { firstField = newField; }
            else { lastField->nextArg = newField; }
            lastField = newField;
        }
