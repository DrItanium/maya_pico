...
struct joinNode *ConstructJoins(
    ...
    if ((startDepth == 1) &&
        ...
        GetPrintWhileLoading(theEnv)) { WriteString(theEnv, STDOUT, "\n"); }
#endif
    ...
    while (theLHS != NULL) {
        ...
        if (theLHS->beginNandDepth == theLHS->endNandDepth) {
            ...
            if ((lastNode->negated == false) &&
                ...
                if (lastLastNode->endNandDepth > theLHS->endNandDepth) { lastNode->beginNandDepth = lastLastNode->endNandDepth; }
                else { lastNode->beginNandDepth = theLHS->endNandDepth; }
    ...
    while (listOfJoins != NULL) {
        ...
        if (listOfJoins->rightSideEntryStructure == rhsStruct) {
            if (TestJoinForReuse(listOfJoins, firstJoin, negatedRHS, existsRHS,
                                 ...
                                 leftHash, rightHash)) { return (listOfJoins); }
        }
...
static struct joinNode *CreateNewJoin(
    ...
    else { newJoin->patternIsNegated = negatedRHSPattern; }
